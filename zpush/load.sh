...
#mkdir -p status/zpush
mkdir -p log

start() {
        #ulimit -c unlimited
        #setsid ./supervise.zpush status/zpush $pythondir/bin/python bin/zpush.py 1>/dev/null  2>log/zpush.log.wf &
        $pythondir/bin/python bin/zpush.py 1>log/zpush.log.wf 2>&1 &
}

stop() {
        ps -ef|grep "$pythondir/bin/python bin/zpush.py"|grep -v grep|awk '{print $2}'|xargs kill -9 > /dev/null 2>&1
        #echo 'dx' > status/zpush/svcontrol
}
