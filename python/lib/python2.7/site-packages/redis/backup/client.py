...
from ctypes import cdll, CDLL, c_double, byref
...
def parse_double(val):
    d = c_double()
    libc.sscanf(val, "%lf", byref(d))
    return d.value
...
class Redis(threading.local):
    ...
    RESPONSE_CALLBACKS = dict_merge(
        ...
        string_keys_to_dict('CARANGE', lambda r: map(parse_double, r)),
    ...
    def execute_command(self, *args, **options):
        ...
        cmds = ['$%s\r\n%s\r\n' % (len(enc_value), enc_value)
                for enc_value in imap(str, args)]
    ...
    def caincrby(self, key, length, index, val):
        return self.execute_command('CAINCRBY', key, length, index, val)

    def cainfo(self, key):
        return self.execute_command('CAINFO', key)

    def carange(self, key, start, end):
        return self.execute_command('CARANGE', key, start, end)
